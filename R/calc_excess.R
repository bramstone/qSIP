...
#'   Specifying a max_label column will return an atom excess value higher than would otherwise be returned. A \code{max_label} value of 1 indicates
  ...
  if(ci_method=='none') {
    ...
    for(i in 1:iters) {
      ...
      if(calc_w_density) {
        ...
        excess <- ((label - light) * npf) / neutron_per_bp
      }
